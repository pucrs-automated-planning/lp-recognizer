...
def average_values(all_domain_data, approach):
	...
	for domain_data in all_domain_data.values():
		if not approach in domain_data.avg_data:
			return None
...
def collect_data(domains, approaches, basepaths, observabilities):
...
def get_latex_content(domains, approaches, basepaths, full):
	print("Tabulating data from %s for domains %s"%(basepaths,domains))
	observabilities = ['10', '30', '50', '70', '100']
	all_domain_data = collect_data(domains, approaches, basepaths, observabilities)
	print(full)
		...
		for obs in observabilities:
			...
			for basepath, approach in zip(basepaths, approaches):
				...
				if full:
					...
					content_table += '& {0} & {1} & {2}'.format(ar, accuracy, spread)
...
def main(template, file, domains, approaches, basepaths):
	# Get content.
	content_table, content_avg = get_latex_content(domains, approaches, basepaths, 'full' in template)
	domains = [name.replace("optimal", "suboptimal") for name in domains]
	content_table_2, content_avg_2 = get_latex_content(domains, approaches, basepaths, 'full' in template)
	# Write latex
	latexContent = ''
