...
class DomainData:
	...
	def read_approach_data(self, file, key):
		...
		for obs in self.observabilities:
			...
			self.obs_data[obs][key] = values
			sum_values = [x + y for x, y in zip(sum_values, values)]
		self.avg_data[key] = [x / len(self.observabilities) for x in sum_values]
...
def collect_data(file, domains, suffix, approaches, observabilities):
	...
	for domain in domains:
		for dt in ['-optimal', '-suboptimal']:
			...
			for approach in approaches:
				path = '../results-new/' + domain_name + suffix + approach + '.txt'
...
def get_latex_content(file, domains):
	...
	approaches1 = ['delta-h-c', 'rg2009', 'lm_hc0']
	approaches2 = ['delta-h-c', 'delta-h-c-f2', 'rg2009', 'lm_hc0']
	all_domain_data = collect_data(file, domains, '-', approaches1, observabilities)
	all_domain_data_noisy = collect_data(file, domains, '-old-noisy-', approaches2, observabilities)
	...
	for domain_name in domains:
		...
		for obs in observabilities:
			...
			content_table += print_agreements(obs, all_domain_data[domain_name + '-optimal'], approaches1)
			content_table += print_agreements(obs, all_domain_data[domain_name + '-suboptimal'], approaches1)
			content_table += print_agreements(obs, all_domain_data_noisy[domain_name + '-optimal'], approaches2)
			content_table += print_agreements(obs, all_domain_data_noisy[domain_name + '-suboptimal'], approaches2)
			content_table += "\\\\"
		content_table += '\hline'
	...
	def print_agreements_avg(domain_data, dt, approaches):
		...
		for approach in approaches:
			...
			content_line += ' & ' + str(ar)
		return content_line
	content_avg += print_agreements_avg(all_domain_data, '-optimal', approaches1)
	content_avg += print_agreements_avg(all_domain_data, '-suboptimal', approaches1)
	content_avg += print_agreements_avg(all_domain_data_noisy, '-optimal', approaches2)
	content_avg += print_agreements_avg(all_domain_data_noisy, '-suboptimal', approaches2)
...
if __name__ == '__main__' :
	...
	file = 'previous-all.tex'
	template = 'previous-all-template.tex'
